                ...
                <tr key={s._id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '4px' }}>
                      ...
                      <button onClick={() => openTransactions(s)} style={{ background: 'var(--bg3)', color: 'var(--accent)', padding: '4px 10px', borderRadius: '4px', fontSize: '11px' }}>العمليات</button>
      ...
      <Modal open={!!payModal} onClose={() => setPayModal(null)} title={`تسديد للمورد: ${payModal?.name}`} width="380px">
        <form onSubmit={handlePay} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ textAlign: 'center', marginBottom: '8px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text2)' }}>المتبقي: <span style={{ color: 'var(--danger)', fontWeight: 'bold' }}>{((payModal?.totalPurchases || 0) - (payModal?.totalPaid || 0)).toFixed(2)}</span></div>
          </div>
