  ...
  return (
    <div style={{ padding: '20px', overflow: 'auto', height: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        ...
        <StatCard icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="18" height="18"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} label="صلاحية" value={summary?.nearExpiryCount ?? '...'} color={'var(--warning)'} />
      </div>
      ...
      <div className="card" style={{ padding: '16px' }}>
        ...
        {summary?.lowStockProducts?.length > 0 ? (
          summary.lowStockProducts.map(p => (
            <div key={p._id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 'bold', color: 'var(--text)', padding: '4px 0', borderBottom: '1px solid var(--bg3)' }}>
                <span>{p.name}</span>
                <span style={{ color: 'var(--danger)' }}>المتبقي: {p.stock} / الحد: {p.reorderPoint}</span>
            </div>
          ))
        ) : (
        ...
        {summary?.nearExpiry?.length > 0 ? (
          summary.nearExpiry.map(p => {
            ...
            return (
              <div key={p._id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 'bold', color: 'var(--text)', padding: '4px 0', borderBottom: '1px solid var(--bg3)' }}>
                <span>{p.name}</span>
                <span style={{ color }}>ينتهي: {p.expiryDate} ({daysLeft > 0 ? `${daysLeft} يوم` : 'منتهية'})</span>
              </div>
            )
          })
        ) : (
