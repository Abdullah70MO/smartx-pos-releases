...
function listProducts(realm, query, limit, page, pageSize) {
  ...
  const mapProduct = p => ({
    ...
    active: p.active, image: p.image || '', expiryDate: p.expiryDate || '', updatedAt: p.updatedAt?.toISOString()
  })
  if (page != null) {
...
function saveProduct(realm, data) {
  ...
  if (data.barcode) {
    const duplicateBarcode = realm.objects('Product').filtered('barcode == $0 AND _id != $1', data.barcode.trim(), data._id || '')[0]
    if (duplicateBarcode) throw new Error('الباركود "' + data.barcode.trim() + '" موجود بالفعل لمنتج "' + duplicateBarcode.name + '"')
  }
  let product
  realm.write(() => {
  ...
  return {
    ...
    active: product.active, image: product.image || '', expiryDate: product.expiryDate || '', updatedAt: product.updatedAt?.toISOString()
  }
}
