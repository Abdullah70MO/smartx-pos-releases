...
- الترخيص: تفعيل الترخيص بمفتاح (license:activate)، تجربة مجانية (license:startTrial)، التحقق الدوري (license:periodicCheck). الترخيص له تاريخ انتهاء ونوع (تجربة/مدفوع). إذا انتهى يدخل في فترة سماح.
- الدعم الفني: بيانات التواصل (يمكنك مراجعة صفحة الإعدادات > الدعم الفني لرؤية أرقام التواصل) — إذا احتاج المستخدم مساعدة تقنية، توجه إلى صفحة الإعدادات > الدعم الفني أو تواصل مع مطور التطبيق عبر الوسائل المتاحة هناك.

=== الأدوات ===
لما يسألك عن بيانات حقيقية، استخدم الأداة المناسبة: get_dashboard_summary, search_products, get_active_shift, get_business_info, get_low_stock_products, search_customers, search_suppliers, search_employees, get_employee_attendance, get_employee_salary, get_employee_sales_today, get_license_info.
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'الحصول على معلومات الترخيص: هل الترخيص مفعل، تاريخ الانتهاء، النوع، الفترة المتبقية',
      parameters: { type: 'object', properties: {}, required: [] }
    }
  }
]
...
async function callTool(name, args) {
  ...
  switch (name) {
    ...
    case 'get_license_info': {
      const lic = realm.objectForPrimaryKey('License', 'license')
      if (!lic) return JSON.stringify({ مفعل: false, رسالة: 'لا توجد معلومات ترخيص' })
      const daysLeft = lic.expiresAt ? Math.ceil((new Date(lic.expiresAt) - new Date()) / (1000 * 60 * 60 * 24)) : null
      return JSON.stringify({
        مفعل: lic.activated,
        المفتاح: lic.activatedKey ? '***' + lic.activatedKey.slice(-4) : '',
        النوع: lic.licenseType || 'تجربة',
        تاريخ_الانتهاء: lic.expiresAt?.toISOString().slice(0, 10),
        الأيام_المتبقية: daysLeft,
        بداية_التجربة: lic.trialStartedAt?.toISOString().slice(0, 10)
      })
    }
    default:
